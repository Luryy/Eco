...
const Points = () => {
  ...
  const [initiaLocation, setInitialLocation] = useState<number[]>([0,0]);
  ...
  useEffect(() => {
    async function loadPosition(){
      ...
      if (status !== 'granted'){
        Alert.alert('Opsssss', 'Precisamos da sua permissão para acesar a localização');
        return
      }
        ...
        <View style={styles.mapContainer}>
          { initiaLocation[0] !== 0 && (
            <MapView
              style={styles.map}
              initialRegion={{
                latitude: initiaLocation[0],
                longitude: initiaLocation[1],
                latitudeDelta: 0.014,
